...
import shutil
from DataSet import CrossDataSet
from losses import DiceLoss,make_one_hot
...
def main():
    ...
    print("build vnet")
    model = UNet(1, 2, width_multiplier=1, trilinear=True, use_ds_conv=False)
    ...
    trainSet = CrossDataSet(root='./data/train_source',txt_name='./data/train_source.txt',train_flag=True)
...
def train_nll(args, epoch, model, trainLoader, optimizer, trainF, weights):
    ...
    for batch_idx, (data, target) in enumerate(trainLoader):
        ...
        dice_loss = dice_error(output, target)
...
def train_dice(args, epoch, model, trainLoader, optimizer, trainF, weights):
    ...
    nTrain = len(trainLoader.dataset)
    criterion = DiceLoss()
    for batch_idx, (data, target, data_name, target_name) in enumerate(trainLoader):
        if args.cuda:
            data, target = data.cuda(), target.cuda()
        data, target = Variable(data), Variable(target)
        optimizer.zero_grad()
        ...
        optimizer.step()
        nProcessed += len(data)
        dice = 100.*(1. - loss.data.item())
        partialEpoch = epoch + batch_idx / len(trainLoader) - 1
        print('Train Epoch: {:.2f} [{}/{} ({:.0f}%)]\tLoss: {:.8f}\tDice: {:.8f}'.format(
            partialEpoch, nProcessed, nTrain, 100. * batch_idx / len(trainLoader),
            loss.data, dice))

        trainF.write('{},{},{}\n'.format(partialEpoch, loss.data, dice))
        trainF.flush()
